...
class ResultsWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.results_table.setHorizontalHeaderLabels(["案號", "項目", "贊成", "反對", "總計", "贊成率(%)", "是否通過"])
    ...
    def refresh_results(self):
        ...
        voting_items = self.db.get_voting_items()

        for row_idx, item in enumerate(voting_items):
            results = self.db.get_voting_results(item['id'])
            ...
            self.results_table.setItem(row_idx, 0, QTableWidgetItem(results.get('case_number', item['case_number'])))
            self.results_table.setItem(row_idx, 1, QTableWidgetItem(results.get('item_name', item['name'])))
    ...
    def merge_device_data(self):
        file_paths, _ = QFileDialog.getOpenFileNames(self, "選擇要合併的數據文件", "", "JSON Files (*.json)")
        if not file_paths:
            return
